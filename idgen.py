...
usage = f"""idgen: A Nintendo GameID generator
usage: {__name__} [-gl]
...
if generate:
    ...
    publishers = []
    for pub in codes.publisher_codes:
        publishers.append(pub["id"])

    print(codes.publisher_codes)
    print(f"which publisher code(default {codes.publisher_codes[1]}):")
    publisher = input()
    if publisher == '' or publisher not in publishers:
        publisher = codes.publisher_codes[1]["id"]
...
if lookup:
    ...
    for tid in title_ids:
        if lookuptitle in tid:
            idsearch = True

    if idsearch == True:
        for title in codes.title_codes:
            if lookuptitle in title["id"]:
                index = title["id"].lower().find(lookuptitle.lower())
                name = title["id"][:index] + "\033[1;32m" + lookuptitle.upper() + "\033[0m" + title["id"][(index + len(lookuptitle)):]

                print(str(title).replace(title["id"], name))
